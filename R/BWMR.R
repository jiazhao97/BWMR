...
BWMR <- function(gammahat, Gammahat, sigmaX, sigmaY) {
  ...
  for (iter in 1:5000) {
    ...
    ## check ELBO and approximate log-likelihood
  ...
  plot2 <- ggplot(df2, aes(x=iteration, y=ELBO_iter)) + geom_line(size = 0.3, color = "tomato1") + geom_point(size=0.3, color = "tomato1") +
  ...
  plot3 <- ggplot(data = df3, mapping = aes(x = factor(serial_number), y = weight, fill = weight)) + geom_bar(stat = 'identity', position = 'dodge') +
    labs(x = "Data Point No.", y = "Weight", title = "Plot3: Estimate of Weight of Each Data Point") +
    ylim(0, 1)
  ...
  ## output
  output <- list(mu_beta, se_beta, P_value, plot1, plot2, plot3, plot4)
}
